      ...
      <div class="relative bg-white shadow-xl">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3">
          ...
          <div class="relative px-6 py-10 overflow-hidden bg-indigo-700 sm:px-10 xl:p-12">
            ...
            <p class="max-w-3xl mt-6 text-base text-indigo-50">Nullam risus blandit ac aliquam justo ipsum. Quam mauris volutpat massa dictumst amet. Sapien tortor lacus arcu.</p>
            <dl class="mt-8 space-y-6">
              <dt><span class="sr-only">Phone number</span></dt>
              <dd class="flex text-base text-indigo-50">
                <!-- <PhoneIcon class="flex-shrink-0 w-6 h-6 text-indigo-200" aria-hidden="true" /> -->
                <span class="ml-3">+(243)992003159</span>
              </dd>
              <dt><span class="sr-only">Email</span></dt>
              <dd class="flex text-base text-indigo-50">
                <!-- <MailIcon class="flex-shrink-0 w-6 h-6 text-indigo-200" aria-hidden="true" /> -->
                <span class="ml-3">muhindosheridan@gmail.com</span>
              </dd>
            </dl>
          ...
          <div class="px-6 py-10 sm:px-10 lg:col-span-2 xl:p-12">
            <h3 class="text-lg font-medium text-gray-900">Laissez nous un message</h3>
            <form action="#" method="POST" class="grid grid-cols-1 mt-6 gap-y-6 sm:grid-cols-2 sm:gap-x-8">
              <div>
                <label for="first-name" class="block text-sm font-medium text-gray-900">Votre nom complet</label>
                <div class="mt-1">
                  <input v-model="contact.name" type="text" name="first-name" id="first-name" autocomplete="given-name" class="block w-full px-4 py-3 text-gray-900 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" />
                </div>
              </div>
              <div>
                <div class="flex justify-between">
                  <label for="phone" class="block text-sm font-medium text-gray-900">Votre numero phone</label>
                  <span id="phone-optional" class="text-sm text-gray-500">Optional</span>
                </div>
                <div class="mt-1">
                  <input v-model="contact.phone" type="text" name="phone" id="phone" autocomplete="tel" class="block w-full px-4 py-3 text-gray-900 border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" aria-describedby="phone-optional" />
                </div>
              </div>              
              <div class="sm:col-span-2">
                ...
                <div class="mt-1">
                  <textarea v-model="contact.message" id="message" name="message" rows="4" class="block w-full px-4 py-3 text-gray-900 border border-gray-300 rounded-md shadow-sm focus:ring-indigo-500 focus:border-indigo-500" aria-describedby="message-max" />
                </div>
              </div>
              <div class="sm:col-span-2 sm:flex sm:justify-end">
                <button @click.prevent="sendMessage" type="submit" class="inline-flex items-center justify-center w-full px-6 py-3 mt-2 text-base font-medium text-white bg-indigo-600 border border-transparent rounded-md shadow-sm hover:bg-indigo-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-indigo-500 sm:w-auto">
                  Submit
                </button>
              </div>
            </form>
